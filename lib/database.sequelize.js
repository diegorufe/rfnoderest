...
class BaseDaoSequelize {
   ...
   * @param {*} fetchs
   */
  async count(filters, fetchs, transaction, transaction) {
  ...
  async destroy(element, transaction) {
    ...
    data.destroy({ transaction: transaction });
    return data;
  }
  ...
  async save(element, transaction) {
    ...
    if (data.dataValues.id != null || data.dataValues.id != undefined) {
      ...
      data = await this.model.findOne({ where: { id: element.id } }, { transaction: transaction });
    } else {
      data = await data.save({ transaction: transaction });
    }
    return data;
  }
