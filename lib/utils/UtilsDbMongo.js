...
class UtilsDbMongo {
  ...
  static startTransaction(transaction) {
    if (UtilsCommons.isNotNull(transaction)) {
      transaction.session.startSession();
   ...
   * @param {*} transaction
   */
  async static commitTransaction(transaction) {
    if (UtilsCommons.isNotNull(transaction)) {
   ...
   * @param {*} transaction 
   */
  async static abortTransaction(transaction) {
    if (UtilsCommons.isNotNull(transaction)) {
  ...
  /**
   * Method for apply transaction 
   * @param {*} transaction
   * @param {*} fn for apply
   */
  static async applyFunctionWithTransaction(transaction, mapPropertiesCreateDb, fn) {
