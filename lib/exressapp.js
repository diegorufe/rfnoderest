...
const ResponseMethod = require("./beans/commons/ResponseMethod");
  ...
  configErrorHandler() {
    ...
    routerHandler[method]((error, req, res, next) => {
      if (error != null && error != undefined) {
        ...
        const responseMethod = new ResponseMethod();
        responseMethod.error = error;
        responseMethod.messageError = error.message;
        responseMethod.errrorCode = MAP_PROPERTIES_STATUS_HTTP.BAD_GATEWAY;

        res.status(MAP_PROPERTIES_STATUS_HTTP.BAD_GATEWAY);
        res.send(responseMethod);
        return;
      }
      next(error);
    });
