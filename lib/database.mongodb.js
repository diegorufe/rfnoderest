...
class BaseDaoMongoDb {
  ...
  async add(element, transaction, mapParams) {
    ...
    const transactionAction = UtilsCommons.isNull(transaction)
      ? UtilsDbMongo.createTransaction(this.mapPropertiesCreateDb)
      : transaction;
    ...
    try {
      ...
      if (commitTransaction) {
        UtilsDbMongo.commitTransaction(transaction);
      }
    } catch (ex) {
      if (commitTransaction) {
  ...
  async createCollectionIfNotExists(transaction, mapParams) {
    ...
    try {
      ...
      if (UtilsCommons.isNull(collectionDb)) {
        const collectionDb = await this.mapPropertiesCreateDb.DB_MANAGER.createCollection(
          this.__getNameCollection(mapParams)
        );
        created = UtilsCommons.isNotNull(collectionDb);
        // Create indexes for collection
        await this.createIndexes(transaction, mapParams);
      }
    } catch (ex) {
      if (commitTransaction) {
        await UtilsDbMongo.abortTransaction(transactionAction);
